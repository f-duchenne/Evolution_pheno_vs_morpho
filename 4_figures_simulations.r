...
png("Figures/Fig_S2.png",width=1200,height=900,res=150)
s1
dev.off();
...
############### FIGURE 2
p1=ggplot()+
stat_smooth(data=indf3,aes(y=mut.strength,x=time,color=trait,fill=trait,linetype=as.factor(rich)),method="loess")+
...
p2=ggplot()+
stat_smooth(data=indf3,aes(y=comp,x=time,color=trait,fill=trait,linetype=as.factor(rich)),method="loess",span=1)+
theme_bw()+theme(axis.line = element_line(colour = "black"),panel.grid.major = element_blank(),panel.border=element_blank(),
...
xlab("Time")+ggtitle("d")+
guides(linetype=guide_legend(override.aes = list(fill = "white",colour="black")))+scale_x_continuous(breaks=c(0,2500,time_end))+facet_wrap(~competition,ncol=1,labeller = label_bquote(italic(c) == .(competition)))+geom_hline(yintercept=0,col="dodgerblue4",size=1.2)
...
ggplot()+
stat_smooth(data=subset(indf3,trait=="morpho"),aes(y=mut.strength/(competition*comp),x=time,color=trait,fill=trait,linetype=as.factor(rich)))+
theme_bw()+theme(axis.line = element_line(colour = "black"),panel.grid.major = element_blank(),panel.border=element_blank(),
panel.grid.minor = element_blank(),panel.background = element_blank(),plot.title=element_text(size=14,face="bold",hjust = 0),legend.position="none",
strip.background=element_rect(fill=NA,color=NA))+
scale_color_manual(values=colo,labels=c("both traits","only morpho","only pheno"))+scale_fill_manual(values=colo,labels=c("both traits","only morpho","only pheno"))+ylab("mutualism/competition")+guides(linetype=guide_legend(override.aes=list(fill=NA,color="black")))+
...
############### FIGURE 3
pl1=ggplot(data=subset(indf2,rich==30 & time %in% c(0,60,250,990,time_end) & competition %in% c(4,6)),aes(x=Dim.1,y=Dim.2,alpha=time,color=trait,group=paste(essai,trait)))+
...
facet_wrap(~competition,ncol=1,labeller = label_bquote(rows=italic(c) == .(competition)))+scale_alpha_continuous(limits = c(0,time_end), breaks = c(0,time_end),range = c(0.05, 1))
...
geom_vline(xintercept=0,linetype="dashed")+
geom_hline(yintercept=0,linetype="dashed")+
geom_point(data=subset(indf2,time==time_end  & competition %in% c(4)),aes(x=Dim.1,y=Dim.2,alpha=time,color=trait,fill=trait,shape=as.factor(rich)))+
...
pl4=ggplot(data=subset(indf2,time==time_end & competition %in% c(4)),aes(x=as.factor(rich),y=wNODF,color=trait))+geom_violin(position=position_dodge(width=0.5),width=1,scale="width")+
...
pl5=ggplot(data=subset(indf2,time==time_end & competition %in% c(4)),aes(x=as.factor(rich),y=modularity,color=trait))+geom_violin(position=position_dodge(width=0.5),width=1,scale="width")+
geom_boxplot(width=0.1,position=position_dodge(width=0.5))+
theme_bw()+theme(axis.line = element_line(colour = "black"),panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.background = element_blank(),plot.title=element_text(size=14,face="bold",hjust = 0),
strip.background=element_rect(fill=NA,color=NA),panel.border=element_blank(),legend.position="none")+
scale_color_manual(values=colo,labels=c("both traits","only morpho","only pheno"))+
...
geom_vline(xintercept=0,linetype="dashed")+
geom_hline(yintercept=0,linetype="dashed")+
geom_point(data=subset(indf3b,time==time_end),aes(x=Dim.1,y=Dim.2,alpha=time,color=trait,fill=trait,shape=as.factor(rich)))+
#geom_point(data=subset(indf3,time==0 & trait!="both"),aes(x=Dim.1,y=Dim.2,alpha=time,shape=as.factor(rich)),color="grey",fill="grey",alpha=0.3)+
#geom_point(data=subset(indf3,time==0 & trait=="both"),aes(x=Dim.1,y=Dim.2,alpha=time,shape=as.factor(rich)),color="black",fill="grey",alpha=0.3)+
scale_shape_manual(values = c(21,22,23))+
theme_bw()+theme(axis.line = element_line(colour = "black"),panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.background = element_blank(),plot.title=element_text(size=14,face="bold",hjust = 0),legend.position="none",
...
png("Figures/Fig_S6.png",width=1200,height=1000,res=150)
pl3b
dev.off();
...
scale_color_manual(values=colo,labels=c("both traits","only morpho","only pheno"))+
labs(linetype=expression(paste(n[sp]," / guild")))+ylab(expression(paste("Structural stability  ",(omega))))+xlab("Time")+facet_grid(cols=vars(rich), labeller = label_bquote(cols=n[sp] == .(rich)))+scale_x_sqrt(breaks=c(0,2500,time_end))+labs(colour="Simulations with:",fill="Simulations with:")+
ggtitle("a")
...
strip.background=element_rect(fill=NA,color=NA),panel.border=element_blank())+
scale_color_manual(values=colo,labels=c("both traits","only morpho","only pheno"))+
labs(linetype=expression(paste(n[sp]," / guild")))+ylab(expression(paste("Structural stability  ",(omega))))+xlab("Time")+facet_grid(cols=vars(rich),rows=vars(competition), labeller = label_bquote(cols=n[sp] == .(rich),rows=italic(c) == .(competition)))+scale_x_sqrt(breaks=c(0,2500,time_end))+labs(colour="Simulations with:",fill="Simulations with:")

png("Figures/Fig_S7.png",width=1200,height=1200,res=150)
pl6sup
dev.off();


#Diversity-stability trade-off
indf5=subset(indf2,time %in% c(0,time_end) & competition %in% c(4,6))
...
indf5$time2=factor(indf5$time,labels=c("initial (t = 0)","coevolved (t = time_end)"))
indf5$time2=factor(indf5$time2,levels=c("coevolved (t = time_end)","initial (t = 0)"))
...
b2$time=0
b2$time[b2$time2=="coevolved (t = time_end)"]=time_end
b2$time=ordered(b2$time,levels=c(0,time_end))
...
ggtitle("c")+scale_x_discrete(breaks=c(0,time_end),labels=c("initial","coevolved"))+scale_alpha_discrete(range=c(0.3,1))
