...
pdf("Figures/Fig.1.pdf",width=8,height=7)
grid.arrange(top,midle,bottom,widths=c(2,1),heights=c(1,1,0.7),layout_matrix = lay)
dev.off();
...
png("Figures/Fig.S1.png",width=1200,height=1000,res=150)
pl3b
dev.off();
...
pl6b=ggplot(data=subset(indf4,rcompetition==5),aes(x=time,y=feas_structure,color=trait,group=paste(time,trait)))+geom_violin(position=position_dodge(width=0.5),width=8,scale="width",show.legend=FALSE)+
...
pl6=ggplot(data=subset(indf4,competition==2),aes(x=time,y=feas_structure,color=trait,group=paste(time,trait)))+geom_violin(position=position_dodge(width=0.5),width=8,scale="width",show.legend=FALSE)+
geom_boxplot(width=3,position=position_dodge(width=0.5))+
theme_bw()+theme(axis.line = element_line(colour = "black"),panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.background = element_blank(),plot.title=element_text(size=14,face="bold",hjust = 0),legend.background = element_blank(),
strip.background=element_rect(fill=NA,color=NA),panel.border=element_blank())+
scale_color_manual(values=colo,labels=c("both traits","only morpho","only pheno"))+
...
png("Figures/Fig.S3.png",width=1200,height=600,res=150)
pl6
dev.off();

############################# FIGURE 4
indf5=subset(indf,time %in% c(0,2000) & competition_feas==competition & rho==0.05)
...
model=glm(feas_structure~rich*time2*trait,data=subset(indf5,competition==5),family=quasibinomial)

b=ggpredict(model,c("rich[10:30]","time2","trait"))
pl7a=ggplot()+
geom_line(data=b,aes(x=x,y=predicted,color=facet,linetype=group))+
geom_point(data=subset(indf5,competition==5),aes(x=rich,y=feas_structure,color=trait,shape=as.factor(time2)),alpha=0.2)+
theme_bw()+theme(axis.line = element_line(colour = "black"),panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.background = element_blank(),plot.title=element_text(size=14,face="bold",hjust = 0),
strip.background=element_rect(fill=NA,color=NA),panel.border=element_blank())+
...
pdf("Figures/Fig.4.pdf",width=6,height=3)
plot_grid(pl7a,pl7b,ncol=2,rel_widths=c(2,1))
dev.off();

############################# FIGURE S4

pl7=ggplot(data=subset(indf5,time==0),aes(x=rich,y=feas_structure,color=trait,alpha=as.factor(competition)))+
...
#scale_color_brewer(palette = "Reds")+
labs(alpha=expression(c))+ggtitle("a",subtitle="Initial networks")+scale_x_continuous(breaks=c(10,20,30))+
scale_alpha_discrete(range = c(0.3, 1))

pl8=ggplot(data=subset(indf5,time==2000),aes(x=rich,y=feas_structure,color=trait,alpha=as.factor(competition)))+
stat_summary(fun.y=mean, geom="line", size = 0.5,position=position_dodge(width=0.5))+
...
labs(linetype=expression(paste(n[sp]," / guild")))+ylab(expression(paste("Structural stability  ",(omega))))+xlab("Number of species per guild")+facet_wrap(~trait)+scale_color_manual(values=colo,guide=FALSE)+scale_fill_manual(values=colo)+
#scale_color_brewer(palette = "Reds")+
labs(alpha=expression(c))+ggtitle("b",subtitle="Coevolved networks")+scale_x_continuous(breaks=c(10,20,30))+
scale_alpha_discrete(range = c(0.3, 1))


plot_grid(pl7,pl8,ncol=1,align="hv")

png("Figures/Fig_S4.png",width=1200,height=900,res=150)
plot_grid(pl7,pl8,ncol=1,align="hv")
dev.off();
