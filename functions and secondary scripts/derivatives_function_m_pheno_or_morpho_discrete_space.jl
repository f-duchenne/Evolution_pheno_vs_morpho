    ...
    for ti in 1:t
        ...
        for i=1:(nbsp_a+nbsp_p)
            ...
            @inbounds function pop_derivative_a(x,y)
                ...
                @inbounds function comp_inter_a(p3)
                    ...
                    for v in 1:nbsp_a
                        ...
                        comp_interactions[v]=similarity
                        overlaps[v]=phen
                    end
                    return comp_interactions, overlaps
                end
                obj=comp_inter_a(p3)
                if trait=="morpho"
                    comp_interactions = obj[1] .*  obj[2]
                else
                    comp_interactions = obj[1]
                end
                comp_interactions[i,] = 1.0
                overlaps = obj[2]
                d_pop= r .+ alpha*sum(mut_interactions .* abund_flower) .- sum(comp_interactions .* abund_poll) .- sum(overlaps .* abund_poll)
                return d_pop
            end
            ...
            @inbounds function pop_derivative_p(x,y)
                ...
                comp_interactions[i,] = 1.0
                overlaps = obj[2]
                d_pop= r .+ alpha*sum(mut_interactions .* abund_poll) .- sum(comp_interactions .* abund_flower) .- sum(overlaps .* abund_flower)
                return d_pop
            end
            ...
            if i< nbsp_a+1 #if pollinator
                ...
                u[(nbsp_a*2+nbsp_p*2+i)]=u[(nbsp_a*2+nbsp_p*2+i)]+(epsilon .* partial_dev_sd_phen)
