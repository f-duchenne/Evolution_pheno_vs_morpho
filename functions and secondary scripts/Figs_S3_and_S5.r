...
datf2=subset(datf,comp==4 & time %in% plyr::round_any(seq(sqrt(0),sqrt(time_end),length.out=30)^2,10))
...
s3_a=ggplot(subset(datf2,time>0 & type3=="mu"),aes(x=time,y=delta+1,color=trait,fill=trait,linetype=type2))+
